...
def click_button(_step, button_text):
    world.click_link(button_text)


@step(u'I should "([^"]*)" the upload modal$')
def element_visible(_step, visible):
    if visible == 'not see':
        assert not world.css_visible('.upload-modal')
    else:
        assert world.css_visible('.upload-modal')


@step(u'I close upload modal by clicking close button$')
def close_modal(_step):
    world.css_find('.close-button').click()
